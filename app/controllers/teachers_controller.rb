class TeachersController < ApplicationController
    def show
      ...
      @review = Review.new
      if @reviews.size != 0
        @note = @reviews.pluck(:rating).inject{ |sum, el| sum + el } / @reviews.size
      end
    ...
    def index
      ...
      @teachers = @teachers.joins(:meeting_locations).where(meeting_locations: {name: params[:location]}) if params[:location].present?
      @teachers = @teachers.joins(:study_levels).where(study_levels: {name: params[:level]}) if params[:level].present?
      @teachers = @teachers.joins(:teacher_profile).where("exp_years >= ?", params[:exp_years]) if params[:exp_years].present?
      @teachers = @teachers.joins(:teacher_profile).order(is_promoted: :desc, name: :asc).paginate(page: params[:page], per_page: 10)
      
      @categories = Subject.all.pluck(:category).uniq
      @subjects_by_category = Subject.all.group_by(&:category)
      @titles = {}
      @categories.each do |category|
        @titles["#{category}"]=Subject.all.group_by(&:category)[category].pluck(:title)
      end
    ...
    private
      def teacher_params
        params.require(:teacher).permit(:name, :email, teacher_profile_attributes: [:education, :exp_years, :price, :meeting_duration_in_minutes, :desc, :date_of_birth, :city, :image])
      end
end
